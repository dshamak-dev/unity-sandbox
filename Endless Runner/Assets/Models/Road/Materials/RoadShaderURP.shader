    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float sidewalkLeftMask = step(uv.x, _SidewalkWidth);
                float sidewalkRightMask = step(1.0 - uv.x, _SidewalkWidth);
                ...
                float borderLineLeft = step(abs(uv.x - _SidewalkWidth), _BorderLineWidth);
                float borderLineRight = step(abs(uv.x - (1.0 - _SidewalkWidth)), _BorderLineWidth);
                float borderLineMask = max(borderLineLeft, borderLineRight);
                
                // Calculate road lines (only on road area)
                float linePattern = frac(uv.y * _LineRepeat);
